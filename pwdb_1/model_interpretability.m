    ...
    for t = 1:T
        ...
        for noise_level = noise_levels
            for rep = 1:n_perturbations
                ...
                sensitivity_scores = [sensitivity_scores; sensitivity];
            end
        end
...
fprintf('Most important channel (occlusion): %s\n', ...
        {'PPG', 'Area'}{find(mean(occlusion_imp,2) == max(mean(occlusion_imp,2)))});
fprintf('Most important time region (occlusion): Window %d\n', ...
        find(mean(occlusion_imp,1) == max(mean(occlusion_imp,1))));
